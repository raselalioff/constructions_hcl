...
odoo.define('project_gantt_native.wk_GanttRenderer', function (require) {
    "use strict";
    
    var AbstractRenderer = require('web.AbstractRenderer');
    var config = require('web.config');
    var core = require('web.core');
    var GanttRow = require('project_gantt_native.wk_GanttRow');
    ...
    var wk_GanttRenderer = AbstractRenderer.extend({
        config: {
            GanttRow: GanttRow
        },
         ...
         * @param {string} params.wk_colorField
         * @param {Object} params.fieldsInfo
         * @param {Object} params.wk_SCALES
        ...
        init: function (parent, state, params) {
            ...
            this.wk_canPlan = params.wk_canPlan;
        ...
        _getSlotsDates: function () {
            var token = this.wk_SCALES[this.state.scale].interval;
            var gap = this.wk_SCALES[this.state.scale].gap;
            var stopDate = this.state.stopDate;
            var day = this.state.startDate;
            var dates = [];
            while (day <= stopDate) {
                dates.push(day);
                day = day.clone().add(gap, token);
            }
            return dates;
        },
        /**
        ...
        _prepareViewInfo: function () {
            return {
                wk_colorField: this.wk_colorField,
                progressField: this.progressField,
                wk_consolidationParams: this.wk_consolidationParams,
                state: this.state,
                ...
                activeScaleInfo: {
                    ...
                    interval: this.wk_SCALES[this.state.scale].cellPrecisions[this.cellPrecisions[this.state.scale]],
                    time: this.wk_SCALES[this.state.scale].time,
                },
            };
        },
        /**
        ...
        async _renderView() {
            ...
            return Promise.all(proms).then(function () {
                ...
                self._replaceElement(QWeb.render('wk_GanttView', {widget: self}));
        ...
        _renderRows: function (rows, groupedBy) {
            ...
            // FIXME: could we get rid of wk_collapseFirstLevel in Renderer, and fully
            // handle this in Model?
            var hideSidebar = groupedBy.length === 0;
            if (this.wk_collapseFirstLevel) {
            ...
            rows.forEach(function (row) {
                ...
                if (groupedBy.length) {
                    ...
                    if(row.priority==='1'){
                        pillsInfo.priority = true;
                        // pillsInfo.gantt_color =  row.gantt_color;
                    }
                    else if(row.priority==='0'){
                        pillsInfo.priority = false;                    
                    }
                ...
                var params = {
                    ...
                    wk_canPlan: self.wk_canPlan,
                    isGroup: row.isGroup,
                    consolidate: (groupLevel === 0) && (self.state.groupedBy[0] === self.wk_consolidationParams.maxField),
                    hideSidebar: hideSidebar,
                    isOpen: row.isOpen,
                    disableResize: disableResize,
                    rowId: row.id,
                    scales: self.wk_SCALES,
        ...
        _renderTotalRow: function () {
            ...
            var params = {
                ...
                wk_canPlan: this.wk_canPlan,
                hideSidebar: this.state.groupedBy.length === 0,
                isGroup: true,
                rowId: '__total_row__',
                scales: this.wk_SCALES,
            };

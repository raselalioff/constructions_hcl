...
odoo.define('project_gantt_native.wk_GanttRow', function (require) {
    "use strict";
    ...
    var wk_GanttRow = Widget.extend({
        template: 'wk_gantt.Row',
        events: {
            'mouseleave': 'wk_onMouseLeave',
            'mousemove .o_gantt_cell': 'wk_onMouseMove',
            'mouseenter .o_gantt_pill': 'wk_onPillEntered',
            'click .o_gantt_pill': '_onPillClicked',
            'click': 'wk_onRowSidebarClicked',
        ...
        init: function (parent, pillsInfo, viewInfo, options) {
            ...
            this.priority = pillsInfo.priority;
            // this.gantt_color = pillsInfo.gantt_color;
            ...
            this.wk_colorField = viewInfo.wk_colorField;
    
            this.options = options;
            this.wk_SCALES = options.scales;
            ...
            this.wk_consolidationParams = viewInfo.wk_consolidationParams;
            ...
            if (this.isGroup && this.pills.length) {
                ...
                if(['hours','hours4','hours2'].includes(this.state.scale) && this.pills[0]){
                    this.pills[0].stopDate = this.pills[0].aggregatedPills[0].stopDate;
                }
            } else {
        ...
        _bindPopover: function () {
            ...
            this.$('.o_gantt_pill').popover({
                ...
                placement: 'top',
                content: function () {
                    return self.viewInfo.popoverQWeb.render('wk_gantt.popover', self._getPopoverContext($(this).data('id')));
                },
            });
        },
        ...
        _aggregateGroupedPills: function () {
            ...
            var timeToken = this.wk_SCALES[this.state.scale].time;
            var precision = this.viewInfo.activeScaleInfo.precision;
            var cellTime = this.wk_SCALES[this.state.scale].cellPrecisions[precision];
            ...
            this.pills = _.reduce(intervals, function (pills, intervalStart) {
                ...
                if (pillsInThisInterval.length) {
                    ...
                    if (isContinuous && previousPill.count === pillsInThisInterval.length) {
                        ...
                        if (self.consolidate && self.wk_consolidationParams.field) {
                            newPill.consolidationValue = pillsInThisInterval.reduce(
                                function (sum, pill) {
                                    if (!pill[self.wk_consolidationParams.excludeField]) {
                                        return sum + pill[self.wk_consolidationParams.field];
                                    }
                                    return sum; // Don't sum this pill if it is excluded
                                },
                                0
                            );
                            newPill.wk_consolidationMaxValue = self.wk_consolidationParams.maxValue;
                            newPill.consolidationExceeded = newPill.consolidationValue > newPill.wk_consolidationMaxValue;
                        }
            ...
            this.pills.forEach(function (pill) {
                ...
                if (self.consolidate && self.wk_consolidationParams.maxValue) {
            ...
            this.pills.forEach(function (pill) {
                switch (self.state.scale) {
                    case 'hours2':
                        ...
                        pill.width = gapSize > 0 ? 'calc(' + (diff / (60*2)) * 100 + '% - ' + 10 + '% + ' + gapSize + 'px)' : (diff / 60) * 100 + '%';
                        break;
                    case 'hours4':
                        left = pill.startDate.diff(pill.startDate.clone().startOf('hour'), 'minutes');
                        pill.leftMargin = (left / 60) * 100;
                        diff = pill.stopDate.diff(pill.startDate, 'minutes');
                        var gapSize = pill.stopDate.diff(pill.startDate, 'hours') - 1; 
                        pill.width = gapSize > 0 ? 'calc(' + (diff / (60*4)) * 100 + '% - ' + 8 + '% + ' + gapSize + 'px)' : (diff / 60) * 100 + '%';
                        break;
                    case 'hours':
                        left = pill.startDate.diff(pill.startDate.clone().startOf('hour'), 'minutes');
                        pill.leftMargin = (left / 60) * 100;
                        diff = pill.stopDate.diff(pill.startDate, 'minutes');
                        var gapSize = pill.stopDate.diff(pill.startDate, 'hours') - 1; 
                        pill.width = gapSize > 0 ? 'calc(' + (diff / (60*8)) * 100 + '% - ' + 6 + '% + ' + gapSize + 'px)' : (diff / 60) * 100 + '%';
                        break;
                    case 'day':
                        left = pill.startDate.diff(pill.startDate.clone().startOf('hour'), 'minutes');
                        pill.leftMargin = (left / 60) * 100;
                ...
                pill.background_color = pill.gantt_color;
            });
        },
        /**
        ...
        */
        ...
        _evaluateDecoration: function () {
            ...
            this.pills.forEach(function (pill) {
                ...
                if (self.wk_colorField) {
                    pill._color = self._getColor(pill[self.wk_colorField]);
                }
        ...
        _insertIntoSlot: function () {
            ...
            var intervalToken = this.wk_SCALES[scale].interval;
            var precision = this.viewInfo.activeScaleInfo.precision;
            var x, y;
        ...
        _updateResizeBadge: function ($pill, diff, ui) {
            ...
            if (diff) {
                var direction = ui.position.left ? 'left' : 'right';
                $( QWeb.render('wk_gantt.ResizeBadge', {
                    diff: diff,
                    direction: direction,
         ...
         * @private
         */
        wk_onRowSidebarClicked: function () {
